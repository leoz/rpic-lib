...
int
rj_open (const char *devbusname, int i2caddr)
{
  int rval;
...
int
rj232_read (unsigned char *buf, int maxlen)
{
  int i = 0;
...
int
rj485_read (unsigned char *buf, int maxlen)
{
  int i = 0;
...
int
rj232_send (unsigned char *buf, int len)
{
  int i, rval;
  for (i = 0; i < len; i++)
...
int
rj485_send (unsigned char *buf, int len)
{
  int i, rval;
  for (i = 0; i < len; i++)
...
int
rj_readblock (int subreg, void *inbuf)
{
  int rval, retry = 3;
  do
    {
...
int
rj_writebyte (int subreg, int data)
{
  int rval, retry = 3;
  do
    {
...
int
rj_writeword (int subreg, int data)
{
  int rval, retry = 3;
  do
    {
